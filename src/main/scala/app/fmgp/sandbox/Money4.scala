...
sealed abstract case class MoneyX[+T <: CurrencyX](amount: BigDecimal) {
  def currency: T
  def currencyName = currency.toString
  def toMoneyX: Money4Map = Money4Map.fromMoney(this) //FUCK YEAH !! +T
}

object MoneyX {
  def apply[T <: CurrencyX](amount: BigDecimal, t:T): MoneyX[T] = new MoneyX[T](amount) { //FIXME need to round
    override def currency: T = t
  }
  implicit val eqv: Eq[MoneyX[_]] = Eq.fromUniversalEquals
}
...
object Money4MapCommutativeGroup {
  implicit val moneyXCommutativeGroup: CommutativeGroup[Money4Map] = new CommutativeGroup[Money4Map] {
...
object Money4Map {
  ...
  def fromMoney(m: MoneyX[CurrencyX]): Money4Map = {
